...
updateLoginDetails: function to asynchronously update login groups and other details
branding: see brandings/README.md
...
branding = require 'branding'   # Special branding package from branding/<some branding>/branding.json
...
collectionNames = ['sites', 'forms', 'groups', 'group_members', 'responses', 'tests', 'source_notes', 'sensors', 'sensor_data', 'sites', 'properties', 'entity_types', 'units']
...
# Adds entity collections to the database if they do not already exist
addEntityCollections = (db, entityTypes) =>
  # Add entity collections
  for col in _.pluck(entityTypes, "code")
    # Only add if do not exist
    if db[col]
      continue

    db.localDb.addCollection(col)
    
    # Override url for entity collections
    db.remoteDb.addCollection(col, { url: apiUrl + "entities/#{col}" })
    db.addCollection(col)

# Adds support for entities by caching properties, units and entity_types at context level
# Also adds entity collections
addEntitySupport = (ctx, success, error) ->
  # Allow success to be called once
  successOnce = _.once(success)

  ctx.db.entity_types.find({}, { interim: true, cacheFind: true }).fetch (entity_types) =>
    ctx.db.units.find({}, { interim: true, cacheFind: true }).fetch (units) =>
      ctx.db.properties.find({}, { interim: true, cacheFind: true }).fetch (properties) =>
        # Set on context
        ...
        # Add databases
        addEntityCollections(ctx.db, entity_types)
...
exports.setupContext = (ctx, success) ->
  login = loginUtils.getLogin(storage)
  if login
    setupLoginContext(login, ctx, success)
  else
    setupAnonymousContext(ctx, success)

# Anonymous context for not logged in
exports.setupAnonymousContext = setupAnonymousContext = (ctx, success) ->
  createDb null, (db) =>
    # Allow nothing
...
exports.setupDemoContext = (ctx, success) ->
  createDb null, (db) =>
    ...
    if ctx.camera? and temporaryFs
      imageAcquirer = {
        acquire: (success, error) ->
          ctx.camera.takePicture (url) ->
            # Add image
...
exports.setupLoginContext = setupLoginContext = (login, ctx, success) ->
  createDb login, (db) =>
    if persistentFs
      fileTransfer = new FileTransfer()
      imageManager = new CachedImageManager(persistentFs, apiUrl, "Android/data/#{branding.package}/images", login.client, fileTransfer)  
    else
      imageManager = new SimpleImageManager(apiUrl)

    auth = new authModule.UserAuth(login.user, login.groups)
    siteCodesManager = new siteCodes.SiteCodesManager({url: apiUrl + "site_codes?client=#{login.client}", storage: ctx.storage})
    ...
    if ctx.camera? and persistentFs
      imageAcquirer = {
        acquire: (success, error) ->
          ctx.camera.takePicture (url) ->
            # Add image
    ...
    # Add function to asynchronously update login details (includes groups)
    ctx.updateLoginDetails = (callback) ->
      # Getting client returns groups and other details in response
      $.getJSON(apiUrl + "clients/" + login.client).done (response) =>
        # Update login and save
        _.extend(login, response)
    ...
    ctx.updateLoginDetails()

    # Setup entity support
    addEntitySupport(ctx, success, error)
